<template>
    <div class="card bg-white text-white">
        ...
        <div v-if="!visibleError404" class="w-12 md:w-10 mx-auto p-2"
        ...
        <div v-if="!visibleError404" class="w-12 md:w-10 mx-auto p-3 md:p-4" :style="{ backgroundColor: companyData?.colorPrimario }">           
            <div class="grid">
                    ...
                    >
                        <template #item="slotProps">
                            <div class="image-container" @click="mostrarFotos = true">
                                <img :src="resolveURLFiles(slotProps.item.urlFoto)" style="width: 100% !important; display: block" />
                        ...
                        <template #thumbnail="slotProps">
                            <img :src="resolveURLFiles(slotProps.item.urlFoto)" style="display: block; width: 50px;" />
                        </template>
                    </Galleria>
                    <Galleria 
                    ...
                    >
                        <template #item="slotProps">
                            <div class="image-container" @click="mostrarVideos = true">
                                <video controls preload="auto" :src="resolveURLFiles(slotProps.item.urlVideo)" class="media-preview"></video>
                                <div class="hover-overlay">
                                    <i class="pi pi-search"></i>
                                </div>
                            </div>
...
const abrirChatWhatsappAsesor = (nombreInmueble: string) => {
    window.open(`https://api.whatsapp.com/send?phone=${companyData?.value?.telefono}&text=Hola,%20me%20interesa%20esta%20propiedad,%20${nombreInmueble}%20${location.href}`,  '_blank')
}
