<template>
    <div class="card bg-white text-white">
        <div class="w-12 md:w-10 mx-auto text-2xl p-4"
        :style="{ backgroundColor: companyData?.colorSecundario }">
            Lista de inmuebles            
            <ButtonPrime 
                type="button" 
                :icon="!visibleFiltrosAvanzados ? 'pi pi-eye' : 'pi pi-eye-slash'"
                v-tooltip="!visibleFiltrosAvanzados ? 'Activar filtros avanzados' : 'Desactivar filtros avanzados'"
                severity="primary"
                @click="activarDesactivarFiltrosAvanzados"
            />
        </div>
        <div class="w-12 md:w-10 mx-auto p-3 md:p-4" :style="{ backgroundColor: companyData?.colorPrimario }">
            <div v-if="visibleFiltrosAvanzados">
                <h2>Filtros avanzados</h2>
                <div class="formgrid grid w-full p-4">
                    <div class="field col-12 md:col-6 lg:col-4 xl:col-3">
                        <InputNumber
                            v-model="filtrosAvanzados.precioDesde"
                            mode="currency"
                            currency="COP"
                            roundingMode="floor"
                            :minFractionDigits="0"
                    ...
                    <div class="field col-12 md:col-6 lg:col-4 xl:col-3">
                        <InputNumber
                            v-model="filtrosAvanzados.precioHasta"
                            mode="currency"
                            currency="COP"
                            roundingMode="floor"
                            :minFractionDigits="0"
                    ...
                    <div class="field col-12 md:col-6 lg:col-4 xl:col-3">
                        <InputNumber
                            v-model="filtrosAvanzados.dormitorios"
                    ...
                    <div class="field col-12 md:col-6 lg:col-4 xl:col-3">
                        <InputNumber
                            v-model="filtrosAvanzados.banos"
                            placeholder="# Baños"
                            fluid
                        />
                    </div>
                    ...
                    <div class="field col-12 md:col-6 lg:col-4 xl:col-3">      
                        <TextareaPrime 
                            v-model="filtrosAvanzados.caracteristicasEspeciales" 
                            rows="6"
            ...
            <div v-if="inmuebles.length === 0" class="text-center text-lg my-4 py-4">
                No se encontraron inmuebles que coincidan con los filtros aplicados. Prueba ajustando los criterios de búsqueda o eliminando algunos filtros para ver más opciones.
                <ButtonPrime
                    class="mt-4"
                    type="button"
            ...
            <div v-else>
                <div class="formgrid grid">
                    <div v-for="(inmueble) in inmuebles" :key="inmueble.id" class="field col-12 md:col-6 lg:col-4 xl:col-3 mt-2">
                        <Card>
                            <template #content>
                                <div class="relative">
                                    ...
                                    <div class="text-center">
                                        ...
                                        <div :style="{ backgroundColor: companyData?.colorSecundario }">
                                            <h3>$ {{ getPrecio(parseFloat(inmueble.precio)) }} COP</h3>
                                        </div>
                                    </div>
                                    <div class="flex mt-4">
                                    ...
                                    <hr>
                                    <div class="flex">
                                        <div class="flex-1 text-left">Baños</div>
                                        <div class="flex-1 text-right">{{ inmueble.cantidadBanos }}</div>
                                    </div>
...
const visibleFiltrosAvanzados = ref(false);
...
onMounted(async () => {
    const filtrosDefault = route.query || {};
    fetchInmuebles(filtrosDefault)
});

const activarDesactivarFiltrosAvanzados = () => {
    visibleFiltrosAvanzados.value = !visibleFiltrosAvanzados.value
}
...
const fetchInmuebles = async(filtrosDefault: any = {}) => {
    ...
    if (status !== 200) showMessage('error', 'Error', 'Por favor vuelva a intentar o comuníquese con nuestro equipo', 10000);
}
