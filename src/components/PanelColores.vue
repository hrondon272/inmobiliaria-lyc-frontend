<template>
    ...
    <div class="formgrid grid w-12 md:w-10 mx-auto p-3 md:p-4" :style="{ backgroundColor: companyData?.colorPrimario }">
        <div class="field col-12 md:col-6 lg:col-4 mt-2 text-center">
            ...
            <ColorPicker v-model="form.colorPrimario" inputId="cp-primario" format="hex" class="mb-4" inline />
        </div>
        <div class="field col-12 md:col-6 lg:col-4 mt-2 text-center">
            <span>{{ colorSecundarioFormateado }}</span>
            <label for="cp-secundario" class="font-bold block mb-2"> Color secundario </label>
            <ColorPicker v-model="form.colorSecundario" inputId="cp-secundario" format="hex" class="mb-4" inline />
        </div>
        <div class="field col-12 md:col-6 lg:col-4 mt-2 text-center">
            <span>{{ colorTerciarioFormateado }}</span>
            <label for="cp-terciario" class="font-bold block mb-2"> Color terciario </label>
            <ColorPicker v-model="form.colorTerciario" inputId="cp-terciario" format="hex" class="mb-4" inline />
        </div>
        <div class="col-12 text-center my-2">
            <ButtonPrime icon="pi pi-save" aria-label="Actualizar colores" label="Actualizar colores"
                @click="onSubmit" />
        </div>
    </div>
</template>
