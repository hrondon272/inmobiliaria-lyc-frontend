<template>
    ...
    >
        ...
        <FileUpload ref="fotoPrincipalRef" mode="basic" accept=".png,.jpg,.jpeg" :maxFileSize="10000000"
            ...
            <template #header="{ chooseCallback, clearCallback, files }">
                <div class="flex flex-wrap justify-between items-center flex-1 gap-4">
                    <div class="flex gap-2">
                        <ButtonPrime @click="chooseCallback()" label="Seleccionar" icon="pi pi-plus"
                            :disabled="files.length > 14" />
                        <ButtonPrime @click="clearCallback()" label="Cancelar" icon="pi pi-times" severity="danger"
...
const onFotosSeleccionadas = async (event: any, esPortada: boolean = false) => {
    ...
    const archivosComprimidos = await Promise.all(event.files.map((file: File) => compressImages(file)));

    if (archivosComprimidos.length) {
        emit('update:listaFotos', archivosComprimidos.map(foto => ({
            foto: foto, 
            esPortada: esPortada
        })))
    }
